...
- Data rule: Structured release-plan data lives in Firestore at `adminProjects/walls-devine`. Instagram drafts and journals live in Firestore markdown docs at `adminProjects/walls-devine/markdownFiles/{collection}--{slug}`. Storage under `admin-projects/walls-devine/{collection}/{slug}.md` is legacy migration input only.
